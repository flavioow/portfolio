...
export function Project() {
  ...
  const paginatedProjects = otherProjects.slice(startIndex, startIndex + ITEMS_PER_PAGE)
    ...
    <section id="projects" className="py-24 bg-background">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-16">
          <h2 className="text-3xl lg:text-5xl font-serif font-light mb-4 text-balance">{t("projects.featuredTitle")}</h2>
        </div>
        ...
        <div className="mb-24">
          <div className="relative">
            ...
            {featuredProjects.map((project, index) => (
              <div key={project.id} className="relative pl-12 pb-16 last:pb-0">
                {/* Timeline dot */}
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                    <div className="space-y-2">
                      <h3 className="text-2xl lg:text-3xl font-serif font-light mb-4">{project.title}</h3>
                      <p className="text-sm text-muted-foreground">{formatDate(project.publishedDate)}</p>
                    </div>
                  ...
                  <div className="flex gap-6 items-start">
                    <div>
                      <p className="text-muted-foreground leading-relaxed">{t(`projects.${project.slug}.description` as any)}</p>

                      <div className="flex flex-wrap gap-3 mt-4">
                        {project.tags.map((tag) => (
                          <div key={tag} className="flex items-center gap-2">
                            <Icon
                              icon={techIcons[tag] || "mdi:code-tags"}
                              className="size-[1.5rem] text-muted-foreground"
                            />
                            <span className="text-sm text-muted-foreground font-mono">{tag}</span>
                          </div>
                        ))}
                      </div>
                    </div>
        ...
        {otherProjects.length > 0 && (
          <div>
            <div className="text-center mb-16">
              <h2 className="text-3xl lg:text-5xl font-serif font-light mb-4 text-balance">{t("projects.moreTitle")}</h2>
            </div>
                  ...
                  <div className="relative w-full h-full">
                    ...
                    <div className="absolute inset-0 p-6 flex flex-col justify-end">
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <h4 className="font-serif font-bold text-xl text-foreground">{project.title}</h4>
                      </div>

                      <p className="text-sm text-muted-foreground mb-4 line-clamp-2">{t(`projects.${project.slug}.description` as any)}</p>
                      ...
                      <div className="flex gap-2">
                        ...
                        <Button size="sm" variant="outline" className="gap-2 backdrop-blur-sm bg-transparent" asChild>
                          <a href={project.github} target="_blank" rel="noopener noreferrer">
                            <Github className="w-4 h-4" />
                          </a>
                        </Button>
