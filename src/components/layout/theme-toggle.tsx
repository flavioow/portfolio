...
function ThemeToggle({ className, onClick, ...props }: ThemeToggleProps) {
  ...
  return (
    <Button
      ...
      aria-label={label}
      className={cn(
        "pointer-events-auto pointer border-0 bg-input/10 hover:bg-input/30 dark:bg-input/30 dark:hover:bg-input/50 text-sm text-muted-foreground hover:text-foreground transition-colors duration-300",
        className,
      )}
      onClick={(event) => {
        onClick?.(event)
